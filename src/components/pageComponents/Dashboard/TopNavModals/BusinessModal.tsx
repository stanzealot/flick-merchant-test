...
const BusinessModal: React.FC<Props> = ({ isOpen, setIsOpen }) => {
  ...
  const handleAccountSelection = async (business: BusinessData) => {
    ...
    try {
      ...
      if (!response.user) {
        return openGlobalNotification({ type: 'error', message: response.msg, description: '' });
      }
      const userData = {
  ...
  return (
    <Modal customWidth={380} closeIcon={null} open={isOpen} onCancel={() => setIsOpen(false)}>
      {isLoading || merchantLoading ? (
        <div className="mb-6 px-3 pt-5">
          <Skeleton.Input active className="w-full h-[50px] rounded-lg" />
          <div className="mt-7">
            {Array.from({ length: 5 }).map((_, index) => (
              <Skeleton.Button key={Number(index)} active className="w-full h-[60px] mb-3" />
            ))}
          </div>
        </div>
      ) : data?.data?.length > 0 ? (
        <div className="mb-6 px-3 pt-5">
          <div className="relative">
            ...
            />
            <CiSearch className="absolute right-4 top-4 text-gray-500" size={20} />
          </div>
                ...
                >
                  <div className="flex items-center">
                    ...
                    <div className="ml-3">
                      ...
                      <p className="text-gray-500 text-xs text-left">Business</p>
                    </div>
                  </div>
                  {merchantData?.data?.business_name === item.business_name && (
                    <Button
        ...
        <div className="mt-6 mb-6 px-7 pt-5 text-center">
          <Image
            ...
            className="mx-auto"
          />
          <h1 className="mt-7 text-gray-900 font-semibold text-xs">No New Business yet</h1>
